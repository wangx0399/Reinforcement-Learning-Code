...
class Replay_Buffer(object):
    def __init__(self, size, frame_history_len):
        ...
        self.frame_history_len = frame_history_len    # 4
    ...
    def store_frame(self, idx, frame):
        ...
        if self.obs is None:
            self.obs      = np.empty([self.size] + list(frame.shape), dtype=np.uint8)     # self.size, 1, 84, 84
            self.action   = np.empty([self.size],                     dtype=np.int32)
            self.reward   = np.empty([self.size],                     dtype=np.float32)
            self.done     = np.empty([self.size],                     dtype=np.bool)

        self.obs[idx] = frame       # initial  self.next_idx = 0
    ...
    def sample(self, batch_size, data_idx):
        """
        ...
        ** Data       **
        ...
        return self._encode_sample(data_idx)

    def _encode_sample(self, idxes):         # idxes is a list
    ...
    def _encode_observation(self, idx):         # output : 4 * 84 * 84
        """
        :param idx:    scalar
        :return:       (4, 84, 84)
        ...
        # if zero padding is needed for missing context or we are on the boundary of the buffer.
        if start_idx < 0 or missing_context > 0:                                 ### * start_idx<0 * 没必要
    ...
    def encode_recent_observation(self, idx):
        ...
        return self._encode_observation(idx % self.size)
